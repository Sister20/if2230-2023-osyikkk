...
void execute_cmd(char *input, char* home) {
  ...
  if (strcmp(cmd[0], "clear") == 0) { 
    if (cmd_length > 0) {
      syscall_user(5, (uint32_t) "clear: too many arguments", KEYBOARD_BUFFER_SIZE, WHITE);
      return;
    }
    clear_screen();
  } else {
    ...
    if (strcmp(cmd[0], "help") == 0) {
      ...
      if (cmd_length > 0) {
        syscall_user(5, (uint32_t) "help: too many arguments", KEYBOARD_BUFFER_SIZE, WHITE);
        return;
      }
      ...
      if (cmd_length > 1) {
        syscall_user(5, (uint32_t) "cd: too many arguments", KEYBOARD_BUFFER_SIZE, WHITE);
        return;
      }
      ...
      if (cmd_length > 0) {
        ...
        // format : whereis <nama file/folder>
        if (cmd_length == 0){
