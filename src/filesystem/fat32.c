...
void create_fat32(void) {
  ...
  init_directory_table(&(driver_state.dir_table_buf), "ROOT\0\0\0\0",
                       ROOT_CLUSTER_NUMBER);
 ...
 *
 *
 * Else, read and cache entire FileAllocationTable (located at cluster number 1)
 * into driver state
 */
...
void write_clusters(const void *ptr, uint32_t cluster_number,
  ...
  write_blocks(ptr, cluster_to_lba(cluster_number),
...
void read_clusters(void *ptr, uint32_t cluster_number, uint8_t cluster_count) {
  read_blocks(ptr, cluster_to_lba(cluster_number),
              cluster_count * CLUSTER_BLOCK_COUNT);
}
  ...
  for (uint32_t i = 0; i < CLUSTER_SIZE / sizeof(struct FAT32DirectoryEntry);
    ...
    if (memcmp(request.name, parent_table.table[i].name, 8) == 0) {
      if (parent_table.table[i].attribute == ATTR_SUBDIRECTORY) {
        ...
        uint32_t curr_cluster = parent_table.table[i].cluster_high << 16 |
                                parent_table.table[i].cluster_low;
...
int8_t read(struct FAT32DriverRequest request) {
  ...
  for (uint32_t i = 1; i < CLUSTER_SIZE / sizeof(struct FAT32DirectoryEntry);
    ...
    if (memcmp(request.name, parent_table.table[i].name, 8) == 0 &&
      ...
      uint32_t cluster_number = parent_table.table[i].cluster_high << 16 |
                                parent_table.table[i].cluster_low;
      ...
      uint32_t j = 0;
      cluster_number = parent_table.table[i].cluster_high << 16 |
                       parent_table.table[i].cluster_low;
      while (cluster_number != FAT32_FAT_END_OF_FILE) {
        read_clusters((struct ClusterBuffer *)(request.buf) + j, cluster_number,
                      1);
        cluster_number = driver_state.fat_table.cluster_map[cluster_number];
        j++;
      }
...
int8_t write(struct FAT32DriverRequest request) {
  ...
  parent_table.table[idx_empty].attribute =
      request.buffer_size == 0 ? ATTR_SUBDIRECTORY : ATTR_ARCHIVE;
  ...
  if (request.buffer_size == 0) {
    ...
    parent_table.table[idx_empty].cluster_high =
        (uint16_t)(((i - 1) >> 16) & 0xFFFF);
    ...
    for (i = 0; i < (request.buffer_size / CLUSTER_SIZE) - 1; i++) {
      driver_state.fat_table.cluster_map[empty_fat_clusters[i]] =
          empty_fat_clusters[i + 1];
    }
    driver_state.fat_table.cluster_map
        [empty_fat_clusters[(request.buffer_size / CLUSTER_SIZE) - 1]] =
        FAT32_FAT_END_OF_FILE;

    // Set parent directory table entry
    memcpy(parent_table.table[idx_empty].ext, request.ext, 3);
    parent_table.table[idx_empty].cluster_high =
        (uint16_t)((empty_fat_clusters[0] >> 16) & 0xFFFF);
    parent_table.table[idx_empty].cluster_low =
        (uint16_t)(empty_fat_clusters[0] & 0xFFFF);

    // Write file content to storage
    if (request.buffer_size > 0) {
      for (i = 0; i < request.buffer_size / CLUSTER_SIZE; i++) {
        write_clusters(((struct ClusterBuffer *)(request.buf) + i),
                       empty_fat_clusters[i], 1);
      }
    }
  }
    ...
    if (memcmp(request.name, parent_table.table[i].name, 8) == 0 &&
      ...
      if (parent_table.table[i].attribute == ATTR_SUBDIRECTORY) {
        ...
        do {
          ...
          cluster_number = driver_state.fat_table.cluster_map[cluster_number];
          driver_state.fat_table.cluster_map[temp_cluster_number] =
              FAT32_FAT_EMPTY_ENTRY;
        } while (driver_state.fat_table.cluster_map[cluster_number] !=
                 FAT32_FAT_END_OF_FILE);
        // Handle EOF cluster
        driver_state.fat_table.cluster_map[cluster_number] =
            FAT32_FAT_EMPTY_ENTRY;
      ...
      // Write updated file allocation table and parent directory table to
      // storage
      write_clusters(&(driver_state.fat_table), FAT_CLUSTER_NUMBER, 1);
      write_clusters(&(parent_table), request.parent_cluster_number, 1);
